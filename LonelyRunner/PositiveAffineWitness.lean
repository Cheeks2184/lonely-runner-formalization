...
/-- A positive rational affine row has a positive witness when its BHK ratios,
common second-phase period, and quantitative approximation error meet the
stated bounded lower-count hypotheses. -/
theorem exists_positiveAffine_witness {N m : ℕ}
  ...
  obtain ⟨t, htpos, _htshift, ht⟩ :=
    exists_affinePhaseTransfer U (fun i => (V i : ℝ)) (p : ℝ) X Y
      (N : ℝ)⁻¹ (((N * (N - 1) : ℕ) : ℝ)⁻¹)
